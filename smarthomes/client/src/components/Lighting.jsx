...
const Lighting = (e) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:9000/api/getLightings");
  ...
  const handleViewReview = (lighting) => {
    if (!loggedUser) {
      navigate("/Login", {
        state: { from: "Please Login to add items to cart" },
      });
  ...
  const handleWriteReview = (lighting) => {
    if (!loggedUser) {
      navigate("/Login", {
        state: { from: "Please Login to add items to cart" },
      });
  ...
  const filteredLightingsData =
    makerFilter == null
      ...
      : lightingsData.filter(
          (lighting) =>
            lighting.productManufacturer.toLowerCase() ===
        ...
        {rows.map((batch, rowIndex) => (
          <div key={rowIndex} style={{ display: "flex" }}>
            {batch.map((lighting, index) => (
              ...
              >
                <h3>{lighting.productName}</h3>
                <strong style={{ color: "greenyellow" }}>
                  $
                  {loggedUser?.usertype === "retailer"
                    ? (lighting.productPrice * 0.6).toFixed(3)
                    : lighting.productPrice.toFixed(3)}
                </strong>
                <ul>
                  <li style={item}>
                    <p style={{ fontStyle: "italic", color: "ThreeDFace" }}>
                    {`This is ${lighting.productName}`}
                    </p>
                    <img
                      style={img}
                      src={`images/${lighting.ProductType}/${lighting.productImage}`}
                      alt=""
                    />
                  </li>
                  <button
                  ...
                  <button
                    style={shopitem}
                    type="button"
                    onClick={() => handleWriteReview(lighting)}
                  >
                    Write Review
                  </button>
                  <button
                    style={shopitem}
                    type="button"
                    onClick={() => handleViewReview(lighting)}
                  >
                    View Review
                  </button>
                </ul>
